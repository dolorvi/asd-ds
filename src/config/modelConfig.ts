...
/** ---------- Age-band priors (prevalence → logit) ---------- */
export type AgeBandKey = "au_5_14" | "au_15_24" | "au_25_39" | "au_overall" | "generic_2";

export const PRIOR_BY_AGE: Record<
  AgeBandKey,
  { label: string; prevalence: number; logit: number }
> = {
...
export const DEFAULT_CONFIG: Config = {
  // Prior is set dynamically from PRIOR_BY_AGE when auto-prior is ON
  prior: 0,
   ...
   * Strategy:
   *  - Average = negative
   *  - Mild    = small negative (neutral-to-low signal)
   *  - Moderate/Severe = positive
   */
  ...
  /** ---------- WISC (context only; no default mapping) ---------- */
  ...
  /** ---------- ABAS-3 (adaptive) ----------
   * Average and above reduce impairment signal
   */
  ...
  /** ---------- Vineland-3 (adaptive; separate from ABAS-3) ---------- */

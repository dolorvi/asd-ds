...
const setInstrumentBand = useCallback(
  ...
  const reportText = useMemo(() => {
    const lines: string[] = [];
    const lik =
      model.p >= 0.7 ? "High likelihood" : model.p >= model.cut ? "Moderate likelihood" : "Low likelihood";
    const cons =
      migdas.consistency === "inconsistent"
        ? "Inconsistent with autism"
        : model.p >= model.cut
        ? "Consistent with autism"
        : "Not currently consistent with autism";
    ...
    config.srs2Domains.forEach((d) => {
      const s = srs2[d.key]?.severity;
      if (s) lines.push(`- ${d.label}: ${s}`);
    });

    const vin = getInstrumentBand("Vineland-3");
    if (vin) {
      lines.push("");
      lines.push(`Vineland-3 composite: ${vin}`);
    }

    lines.push("");
    lines.push(
      "MIGDAS: " +
        (migdas.consistency === "unclear"
          ? "Unclear"
          : migdas.consistency === "consistent"
          ? "Consistent with autism"
          : "Inconsistent with autism")
    );
    migdas.notes
    ...
    lines.push("");
    lines.push("Governance: Decision support only; clinician judgement prevails.");
    return lines.join("\n");
  }, [model, migdas, supportEstimate, config.srs2Domains, srs2, recommendation, getInstrumentBand]);

  // ---------- categorical wording for sidebar print ----------
  const likelihoodBand = useMemo(() => {
    if (model.p >= 0.7) return "High likelihood";
...
/>
      ...
      <div
        ...
        style={{
          margin: "12px 0",
          padding: 12,
          borderLeft: datasetStatus.passes ? "4px solid #10b981" : "4px solid #f59e0b",
        }}
      >
        ...
        <aside id="summary-section" className="summary-only" style={{ width: 340, position: "sticky", top: 64 }}>
          <Card title="Summary">
            ...
            <div className="print-only" style={{ display: "none" }}>
              ...
              <div>
                <b>Likelihood:</b> {likelihoodBand}
              </div>
              <div>
                <b>Overall pattern:</b> {consistencyBand}
              </div>
              <div style={{ marginTop: 8 }}>
                <b>Provisional support needs:</b> {supportEstimate}
              </div>
            ...
            <div className="row" style={{ gap: 8, marginTop: 12 }}>
              <button style={{ flex: 1 }} onClick={exportSummary}>
                Export summary
              </button>
          ...
          <Card title="Clinician & Governance">
            ...
            <div className="small" style={{ opacity: 0.9 }}>
              Baseline prevalence: {(PRIOR_BY_AGE[ageBand].prevalence * 100).toFixed(1)}% • Log-odds:{" "}
              {PRIOR_BY_AGE[ageBand].logit.toFixed(3)}
            </div>
